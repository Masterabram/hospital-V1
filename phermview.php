...
<?php
  $time = time();
   if (isset($_POST['exit_patient'])){
      // pre($_POST);

      // $time(oid) = time();
      $visit_id = $_POST['visit_id'];
      $phermacy = $_SESSION['name'];

    
      
      mysqli_select_db($connection, $database) or die(mysql_error());

     $query = "UPDATE visits SET checkout={$time}, phermacy='{$phermacy}' ";
     $query .= " WHERE visit_id ='{$visit_id}' ";
      ...
      $_SESSION['message'] = "{$ID} checked out!";
      // echo $patient_id;
        ...
        <p>&nbsp;</p>
                ...
                ?>
                  <ul class="list-group">
                        ...
                        <li class="list-group-item">VISIT ID<strong> :<?php echo $visit['visit_id']; ?></strong></li>
                     
                        <li class="list-group-item">TIME IN<strong> :<?php echo $visit['timestamp']; ?></strong></li>
                     
                        <li class="list-group-item">TIME OUT<strong> :<?php echo date('d-F-Y g:i:s A', $time); ?></strong></li>
                      
                        <li class="list-group-item">Prescription <strong>: <?php echo $visit['prescription']; ?> </strong></li>
                    
              </ul>

     <form method="post" action="phermview.php">
     <input type="hidden" name="visit_id" value="<?php echo $visit['visit_id']; ?>"/>
     <input type="hidden" name="time" value="<?php echo $time; ?>"/>

        <table width="95" border="0" align="center">
              <tr>
                <th width="85" scope="row"><input name="exit_patient" type="submit" class="btn-primary" value="Exit patient"></th>
              </tr>
        </table>
...
<table width="350" border="0">
          ...
          <p>&nbsp;</p>
          <table width="350" class="table-striped" align="centre">
            

                 <?php
                    $visit_set = find_pherms(1, $current_page);
                  // pre($patient_set);
                    while($visit = mysqli_fetch_assoc($visit_set)){
                  ?>
                    <tr>
                      <td width="70"><a href="phermview.php?visit_id=<?php echo $visit['visit_id']; ?>"> <?php echo ($visit['ID']); ?> </a></td>
        ...
        <hr>
    <?php
      $pages = ceil(mysqli_num_rows(find_pherms(0, $current_page)) / 20);
      for($page=1;$page<=$pages;$page++){
    ?>
      <a href="phermview.php?page=<?php echo $page?>"><?php echo $page; ?></a>
    <?php
      }
    ?> 
